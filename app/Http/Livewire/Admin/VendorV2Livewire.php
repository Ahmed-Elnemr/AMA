...
class VendorV2Livewire extends Component
{
    use WithPagination;
    use WithFileUploads;
    ...
    {
        $businessInfo = DB::table('business_information')
            ->join('users', 'business_information.user_id', '=', 'users.id')
            ->join('categories', 'business_information.categories_id', '=', 'categories.id')
            ->leftjoin('media', function ($join) {
                $join->on('business_information.cover_media_id', '=', 'media.id')
                    ->on('business_information.logo_media_id', '=', 'media.id');
            })
            ->select('users.*', 'business_information.*', 'categories.*', 'media.*')

            ->where('users.name', 'like', '%' . $this->search . '%')
            ->orderBy('users.id', 'DESC')->paginate(10);
        // $businessInfo = BusinessInformation::where('name', 'like', '%' . $this->search . '%')->orderBy('id', 'DESC')->paginate(10);
        dd($businessInfo);
        $main_categories = MainCategory::all();
        $categories = Category::all();
        return view('livewire.admin.vendor-v2-livewire', [
